...
public class StringUtil {
	 ...
	 *             <ul>
	 *             <li> {@code CharMatcher.anyOf(what).trimFrom(str)} if
	 *             {@code left == true} and {@code right == true}
	 *             <li> {@code CharMatcher.anyOf(what).trimLeadingFrom(str)} if
	 *             {@code left == true} and {@code right == false}
	 *             <li> {@code CharMatcher.anyOf(what).trimTrailingFrom(str)} if
	 *             {@code left == false} and {@code right == true}
	 *             </ul>
	 */
	@Deprecated
	 ...
	 * @deprecated ensure the string is not null and use
	 *             {@code CharMatcher.LEGACY_WHITESPACE.trimLeadingFrom(str)};
	 *             also consider whether you really want the legacy whitespace
	 *             definition, or something more standard like
	 *             {@link CharMatcher#WHITESPACE}.
	 */
	@Deprecated
	 ...
	 * @return String the striped string
	 * @deprecated ensure the string is not null and use
	 *             {@code CharMatcher.LEGACY_WHITESPACE.trimTrailingFrom(str)};
	 *             also consider whether you really want the legacy whitespace
	 *             definition, or something more standard like
	 *             {@link CharMatcher#WHITESPACE}.
	 */
	...
	private static final String[] UNSAFE_TAGS = { "script", "style", "object", "applet", "!--" };
	...
	private static int javaScriptUnescapeHelper(String s, int i, StringBuilder sb) {
		if (i >= s.length()) {
			throw new IllegalArgumentException("End-of-string after escape character in ["
				+ s
				+ "]");
		}
		...
		case 'u':
			...
			break;
		default:
			throw new IllegalArgumentException("Unknown escape code ["
				+ c
				+ "] at index "
				+ i
				+ " in ["
	...
	/**
	 * Escape a string that is meant to be embedded in a CDATA section. The
	 * returned string is guaranteed to be valid CDATA content. The syntax of
	 * CDATA sections is the following: <blockquote>
	 * <code>&lt;[!CDATA[...]]&gt;</code> </blockquote> The only invalid
	 * character sequence in a CDATA tag is "]]&gt;". If this sequence is
	 * present in the input string, we replace it by closing the current CDATA
	 * field, then write ']]&amp;gt;', then reopen a new CDATA section.
	 */
	public static String xmlCDataEscape(String s) {
		...
		// For each occurrence of "]]>", append a string that adds "]]&gt;"
		// after
		// the end of the CDATA which has just been closed, then opens a new
		// CDATA.
		StringBuilder sb = new StringBuilder();
		int prev = 0;
		do {
	 ...
	 */
	public static Collection<String> string2Collection(String in, String delimiter,
			boolean doStrip, Collection<String> collection) {
		if (in == null) {
			return null;
		}
		if (collection == null) {
	 ...
	 * @return HashMap
	 */
	public static HashMap<String, String> string2Map(String in, String delimEntry, String delimKey,
	 ...
	 */
	public static String arrayMap2String(Map<String, String[]> map, String keyValueDelim,
			String entryDelim) {
	 ...
	 */
	public static String expandShardNames(String dbSpecComponent) throws IllegalArgumentException,
		...
		if (matcher.find()) {
			try {
				...
				int minShard =
						Integer.parseInt(dbSpecComponent.substring(matcher.start(2), matcher.end(2)));
				int maxShard =
						Integer.parseInt(dbSpecComponent.substring(matcher.start(3), matcher.end(3)));
				...
				if (minShard > maxShard) {
					throw new IllegalArgumentException(
							"Maximum shard must be greater than or equal to " + "the minimum shard");
				}
				StringBuilder tmp = new StringBuilder();
				...
				throw new IllegalArgumentException("Malformed DB specification component: "
					+ dbSpecComponent);
			}
		} else {
