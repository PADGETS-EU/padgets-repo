...
class NodeCache(object):
    ...
    def addNode(self, name, klass, node=None):
        print "Loading %s as %s" % (name, klass)
...
class PublishSubscribe(object):
    ...
    def __init__(self, xmpp, dbfile, settings, rest, overridedefault=None):
        ...
        self.xmpp.add_event_handler("got_presence_probe",self.handleXMPPPresenceProbe)
    ...
    def handleXMPPPresenceSubscription(self, subscription) :
        ...
        if subscription['type'] == 'subscribe' :
            print ('someone needs a presence subscription!')
            userJID = subscription['from']
            print(str(userJID))
            #self.xmpp.send_presence_subscription(pto=str(userJID),ptype='subscribed')
            #self.xmpp.send_presence(pto=str(userJID),ptype='online!')
            self.xmpp.sendPresenceSubscription(pto=str(userJID), ptype='subscribe',pnick='the node')
            self.xmpp.sendPresenceSubscription(pto=str(userJID),ptype='subscribed')
    
    def handleXMPPPresenceProbe(self,event):
        self.xmpp.sendPresence(event['from'])
    
    def start(self, event):
        if self.dbfile is not None:
            self.db = PubsubDB(self.dbfile, self.xmpp)
        self.loadNodes()
        self.createNode('__stats__', config={'pubsub#presence_based_delivery': True, 'pubsub#expire': 'presence','pubsub#node_type':'leaf'}, who=None, use_db=False)
        self.createNode('__purgatory__', config={'pubsub#presence_based_delivery': True, 'pubsub#expire': 'presence','pubsub#node_type':'leaf'}, who=None, use_db=False)
        #self.xmpp.send_presence_subscription('test3@gic', ptype='subscribed')
        #self.xmpp.sendPresence('panos@gic/spark')
        #self.xmpp.send_message('panos@gic/spark','initial message! - session started')
        #self.xmpp.send_presence(pstatus = 'online!')
        self.xmpp.schedule('sleekpubsub_generate_stats', 3.0, self.generateStats, repeat=False)
    ...
    def handleIncomingMessage(self,message):
        ...
        b = str(body)
        print 'le yo'
        admins = ['panos@gic/lab','panos@gic/spark']
        for admin in admins :            
            if (from_ == admin) and (b[0:7] == 'connect'):
                print 'hihihi'
                self.xmpp.sendMessage(str(from_),'subscribe:<node> <user jid> / unsubscribe:<node> <user jid> / publish:<node> <message>')
                self.admin_loggedin = True
                try:
                    self.admin = Admin()
                except:
                    print 'failed'
            if (from_ == admin) and (b[0:10] == 'disconnect'):
                self.admin_loggedin = False
        if self.admin_loggedin == True :                                                    
            if b[0:25] == 'create new campaign node:' :
                print 'trying to create the node..'  
                self.admin.create_node(self,b[25:])                                         
            if b[0:12] == 'delete node:' :                    
                self.admin.delete_node(self,b[12:])
            if b[0:10] == 'subscribe:':  
                b = False
                for each_allowed_sub in self.admin.subscribers :  
                    if each_allowed_sub == str(b[16:]):
                        self.admin.subscribe(self,b[10:15],str(b[16:]))
                        b = True
                if b == False : 
                    print 'sorry, subscriber is blacklisted'
            if b[0:17] == 'subscribe to all:':
                k = False
                for each_allowed_sub in self.admin.subscribers :  
                    if each_allowed_sub == str(b[17:]):
                        self.admin.subscribe_to_all_nodes(self,str(b[17:]))
                        k = True
                if k == False : 
                    print 'sorry, subscriber is blacklisted'    
            if b[0:12] == 'unsubscribe:':
                self.admin.unsubscribe(b[12:17],b[18:])
            #if b[0:14] == 'add publisher:':
            #    self.admin.register_publisher(str(b[14:19]),str(b[20:]))
...
#[node] = self.node_classes.get(node_type, BaseNode)(self, self.db, node)
    ...
    def publish(self, node, item, id=None, who=None):
        ...
        node = self.nodes.get(node)
        print ("step 1 publishing in node :",node.name)
    ...
    def createNode(self, node, config=None, who=None, use_db=False):
        if config is None:
        ...
        nodeclass = self.node_classes.get('leaf')
        if node in self.nodes or nodeclass is None:
            return False
        if who:
            who = self.xmpp.getjidbare(who)
        self.nodes.addNode(node, nodeclass, nodeclass(self, db=self.db, name=node, config=config, owner=who, fresh=True, use_db=use_db))
        return True
