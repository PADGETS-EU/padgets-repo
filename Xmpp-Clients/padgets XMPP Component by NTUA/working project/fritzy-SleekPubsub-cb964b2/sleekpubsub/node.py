...
class BaseNode(object):
    ...
    def __init__(self, pubsub, db, name, config=None, owner=None, fresh=False, use_db=False):
    ...
    def dbLoad(self):
        ...
        if not self.fresh:
            ...
            self.db._createNode(self.name, self.config, self.affiliations, self.items)
        ...
        if (
            ...
            if self.config['pubsub#send_last_published_item'] in ('on_sub', 'on_sub_and_presence'):
                if len(self.itemorder) > 0:
                    ...
                    print ("notify event in node-subscribing")
    ...
    def publish(self, item, item_id=None, options=None, who=None):
        print ("step 2 publishing in node")
    ...
    def _publish(self, item, item_id=None, options=None, who=None):
        print ("step 3 publishing in node")
    ...
    def notifyItem(self, event, filterjid=None):
        ...
        for itemi in event.item:
            item_id = itemi.name
            pl = str(itemi.payload)
            payload = ET.Element('{'+pl+'}body')            
        ...
        if payload.tag == '{jabber:client}body':
            ...
            for toset in self.eachSubscriber(filterjid=filterjid): 
                ...
                if not event.hasJid(jid):
                    event.addJid(jid)
                msg['to'] = jid
                msg['from'] = 'test2.gic' #mto or self.xmpp.boundjid
                #self.xmpp.send(msg)           
                print ("subscriber is :",jid)        
                stor = str(jid)
                print ("sending...")    
                self.xmpp.send_message(stor,pl)
...
class QueueNode(BaseNode):
    ...
    def notifyItem(self, event):
        ...
        print ("notify item method in queuenode")
        self._broadcast()
